...
function SearchFilters({cataloguesData, ...props}: SearchFiltersPropsType) {
    ...
    const [selectValue, setSelectValue] = useState<string | null>(form.values.catalogues)
        ...
        <Card className={style.searchFilters} padding="20px" radius="md" withBorder>
            <form onSubmit={form.onSubmit(onSubmitHandler)}>
                <div className={style.group}>
                    ...
                    >Сбросить все
                        <IconX className={style.iconX} height={16}/>
                    </UnstyledButton>
                </div>
                ...
                <Select
                    ...
                    data={cataloguesData.map((m) => {
                        return {value: String(m.key), label: m.title}
                    })}
                ...
                <div className={style.smallScreenGroup}>
                    <NumberInput
                        ...
                        rightSection={<div className={style.numberInputIcons}>
                            <IconChevronUp className={style.icon} size="0.8rem"
                                           onClick={() => {
                                               salaryFromHandler.current?.increment()
                                           }}/>
                            <IconChevronDown className={style.icon} size="0.8rem"
                                             onClick={() => {
                                                 salaryFromHandler.current?.decrement()
                                             }}/></div>}
                    ...
                    <NumberInput
                        ...
                        rightSection={<div className={style.numberInputIcons}>
                            <IconChevronUp className={style.icon} size="0.8rem"
                                           onClick={() => {
                                               salaryToHandler.current?.increment()
                                           }}/>
                            <IconChevronDown className={style.icon} size="0.8rem"
                                             onClick={() => {
                                                 salaryToHandler.current?.decrement()
                                             }}/></div>}
                        className={`${style.input} ${style.inputLast}`}
                        placeholder="До"
                        max={2000000}
